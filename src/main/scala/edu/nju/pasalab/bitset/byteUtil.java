package edu.nju.pasalab.bitset;

import java.io.DataOutputStream;
import java.io.IOException;

/**
 * Suppose data store as big endian
 * byte : 1
 * 假设数据存储是以大端模式存储的：
 * byte: 字节类型 占8位二进制 00000000
 * char: 字符类型 占2个字节 16位二进制 byte[2]
 * int : 整数类型 占4个字节 32位二进制 byte[4]
 * long: 长整数类型 占8个字节 64位二进制 byte[8]
 * float: 浮点数(小数) 占4个字节 32位二进制 byte[4]
 * double: 双精度浮点数(小数) 占8个字节 64位二进制 byte[8]
 *
 * Created by YWJ on 2016.12.22.
